...
Added new CLI feature "reconfigure" (invoked via `output reconfigure`), which realigns npm scripts at the host project's `package.json` with the canonical scripts from `@outputai/cli`:
