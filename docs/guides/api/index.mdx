...
| `error` | Serialized error object on failure, otherwise `null`. It includes `name` and `message` and may include `activityType`, `cause`, and additional diagnostic properties. |

When a workflow fails, the result includes `status: "failed"`, `output: null`, and a structured `error`. Trace metadata may still be present when available.

Results from workflows started by older workers retain the legacy unversioned response shape until those executions leave Temporal retention.
