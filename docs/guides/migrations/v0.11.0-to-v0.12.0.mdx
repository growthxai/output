...
This guide covers breaking changes in `@outputai/llm`. Generation APIs drop native AI SDK call arguments. Skills load only from prompt frontmatter. Call-argument tools merge with prompt YAML tools. LLM start-trace details use `promptFile` and a loaded `prompt` object.
...
`generateImage` `mask` still requires `images`. `Agent.stream()` now appends to `conversationStore` in its wrapped `onFinish` when `finishReason` is not `'error'`.
...
- Expect `Agent.stream()` to persist conversation history on success.
