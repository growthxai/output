...
The emitted component names changed:

- `input_cached` -> `input_cache_read`
- `reasoning` -> `output_reasoning`
- `input_cache_write` is new

The `input` and `output` names are unchanged. `response.cost` is `null`, and no `cost:llm:request` event is emitted, when no priced usage can be calculated. Raw AI SDK usage remains available on the response and trace.
...
- Rename `LLMUsage` components `input_cached` to `input_cache_read` and `reasoning` to `output_reasoning`; handle the new `input_cache_write`.
