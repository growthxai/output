...
description: "Upgrading Output.ai projects from v0.11.0 to v0.12.0: prompt-only skills, prompt-owned maxSteps, merged tools, loaded prompt shape, LLM traces, and aiSdk re-exports in @outputai/llm."
---

This guide covers breaking changes in `@outputai/llm`. Skills and `maxSteps` now load only from prompt frontmatter. Call-argument tools merge with prompt YAML tools. Native prompt tools use the same `maxSteps` loop ceiling as other tools. LLM start-trace details use `promptFile` and a loaded `prompt` object.
...
## `maxSteps` lives on the prompt file

`generateText`, `streamText`, `generateTextWithStreaming`, and `Agent` no longer accept a `maxSteps` argument. Passing it throws:

```
maxSteps must be set in the prompt file, not as a call argument
```

Set it in frontmatter. The default after load is **10**. Call-argument `stopWhen` still overrides it.

Any merged tools, including prompt-only Vertex `googleSearch` / `urlContext`, now get `stopWhen: stepCountIs(maxSteps)` from that prompt value.

Previously the ceiling applied only when call-argument tools or `load_skill` were present. YAML-only grounding stayed at the AI SDK default of one step.
...
- Delete `maxSteps` from those same calls; set `maxSteps` in prompt frontmatter when the default of 10 is wrong.
