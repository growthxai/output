...
description: "Upgrading Output.ai projects from v0.11.0 to v0.12.0: prompt-only skills, merged tools, and loaded prompt shape changes in @outputai/llm."
---

This guide covers breaking changes in `@outputai/llm`. Skills now load only from prompt frontmatter. Call-argument tools merge with prompt YAML tools. Native prompt tools use the same `maxSteps` loop ceiling as other tools.
