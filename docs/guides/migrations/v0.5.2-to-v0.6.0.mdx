...
- The `on()` hook envelope now contains `eventId`, `workflowDetails`, `activityInfo`, and `outputActivityKind`.
...
Events emitted with `emitEvent()` still include your custom payload and `eventId`. When emitted from an Output activity, the contextual fields are now `activityInfo`, `workflowDetails`, and `outputActivityKind`.
...
The `on()` hook now lets you type event-specific attributes directly through its generic. Core still owns the common envelope (`eventId`, `workflowDetails`, `activityInfo`, and `outputActivityKind`); event-emitting packages own their event attribute types.
