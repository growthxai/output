...
| In-flight work outlives the drain | Nothing bounds it, so the worker waits until the platform sends `SIGKILL` and the hook flush is never reached. Exit code is whatever the platform reports | Stops draining at `TEMPORAL_SHUTDOWN_FORCE_TIME`, warns with `Stopping Worker error`, flushes hooks. Exits `1` |
...
| The worker fails to stop | Drains what it can, then reports the failure. Exits `1` | Unchanged |
...
If you alert on non-zero worker exits, the situations that produce one changed. A drain that outlives its window is now one of them, where v0.12.0 hung until the platform stepped in, so expect an exit `1` from any deploy that lands on work longer than `TEMPORAL_SHUTDOWN_FORCE_TIME`. Nothing in the logs separates it from another failure to stop: the `Stopping Worker error` warning reports `IllegalStateError: Worker still in use`, because the forced termination leaves in-flight polls behind, and Temporal's own `Worker failed` line serializes its error as empty. Treat a non-zero exit on a deploy you triggered as routine. An uncaught error still exits `1`, but now only after draining.
