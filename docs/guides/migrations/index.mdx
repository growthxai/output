...
<CardGroup cols={1}>
  ...
  <Card title="v0.10.0 → v0.11.0" href="/migrations/v0.10.0-to-v0.11.0">
    Workflow results and error handling, parsed component schemas, Temporal worker cutover, and other breaking changes.
  </Card>
  <Card title="v0.11.0 → v0.12.0" href="/migrations/v0.11.0-to-v0.12.0">
    LLM call signatures, strict prompt body parsing, loaded prompt shape, `aiSdk` re-exports, and prompt-only skills in `@outputai/llm`.
  </Card>
  <Card title="v0.12.0 → v0.13.0" href="/migrations/v0.12.0-to-v0.13.0">
    Worker shutdown defaults, worker exit codes, and the catalog startup gate in `@outputai/core`, plus the LLM pricing snapshot in `@outputai/llm`.
  </Card>
  <Card title="v0.13.0 → v0.14.0" href="/migrations/v0.13.0-to-v0.14.0">
    Usage and cost reporting for failed and aborted LLM calls in `@outputai/llm`, plus new trace node ids for steps and activities in `@outputai/core`.
  </Card>
  <Card title="v0.14.0 → v0.15.0" href="/migrations/v0.14.0-to-v0.15.0">
    `@outputai/credentials` folded into `@outputai/core/credentials`, removed `onBeforeWorkerStart` hook, and non-retryable credentials errors.
  </Card>
</CardGroup>
