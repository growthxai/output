...
List skill paths in the prompt YAML frontmatter. Paths can be individual `.md` files or directories of `.md` files, resolved relative to the prompt file. A `skills/` folder next to the prompt is not loaded unless you list it.
...
Do not pass `skills` to `generateText`, `streamText`, `generateTextWithStreaming`, or `Agent`. That argument is rejected. Put the paths on the prompt instead. Skills add a `load_skill` tool, so set `maxSteps` in the same frontmatter when the default of 10 is wrong.
