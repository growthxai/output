...
The rendered body is then parsed as either [plain instructions or role-tagged messages](/prompts#prompt-body-modes). A template that emits plain text first selects instruction mode; one that emits a tag first selects message mode.
...
Do not add Liquid's `escape` filter for this safety behavior; it would escape a value that Output already protects. Use `{% raw %}...{% endraw %}` when the authored template itself must contain literal Liquid delimiters such as `{{ example }}`.

In message mode, authored examples of the surrounding role tag must still be escaped. For example, write `&lt;user&gt;example&lt;/user&gt;` inside a `<user>` block. See [Prompt Body Modes](/prompts#prompt-body-modes).
...
| `escape` | `{{ html \| escape }}` | HTML-escaped string (body variables are already protected during prompt parsing) |
