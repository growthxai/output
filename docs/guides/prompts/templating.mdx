...
The rendered body is then parsed as either [plain instructions or role-tagged messages](/prompts#prompt-body-modes). A template that emits plain text first selects instruction mode; one that emits a tag first selects message mode. Text generation APIs require message mode, while `generateImage` requires instruction mode.
