...
`generateText` also supports [skills](/prompts/skills) for on-demand instruction loading. List skill paths in the prompt frontmatter. Do not pass `skills` as a call argument. Set `maxSteps` in the same frontmatter when the default of 10 is wrong.
