...
| `maxSteps` | Tool-loop iterations when tools or skills are present (default 10) | `5` |
| `skills` | Skill file or directory paths, relative to this prompt | `./skills`, `../shared/tone.md` |
...
- Shared top-level fields: `temperature`, `maxTokens`, `maxSteps`, and the image fields (`n`, `size`, `aspectRatio`, `seed`)
...
The tool block is for tool/function call results in multi-turn conversations. Same split as `<assistant>`: included for `generateText` / `streamText`, dropped by `Agent`. Agent conversation history is `messageStore`, not these tags. See [Agents](/packages/llm#message-store).
...
The only supported attribute on a role tag is `options`, a space-separated list of names from frontmatter `messageOptions`. At load, those sets are merged onto the message as `providerOptions`. Any other attribute throws. See [Prompt Caching](/packages/llm#prompt-caching).
