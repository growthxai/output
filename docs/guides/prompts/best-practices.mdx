...
The examples below use [message mode](/prompts#prompt-body-modes): the rendered body starts with a role tag, and all top-level content stays inside supported role blocks. Text generation APIs require message mode. Use instruction mode for image prompts or when consuming `loadPrompt()` results directly.
...
When you need to parse the result programmatically, use `generateText` with `aiSdk.Output.object()` and a Zod schema instead of asking for JSON in the prompt.
...
```typescript
const { output: company } = await generateText({ prompt: 'extract_company@v1', output: aiSdk.Output.object({ schema: companySchema }), ... });
const { output: competitors } = await generateText({ prompt: 'find_competitors@v1', output: aiSdk.Output.array({ element: competitorSchema }), ... });
```
...
| Typed object | `aiSdk.Output.object({ schema })` | Extracted data, evaluator judgments |
| List of items | `aiSdk.Output.array({ element })` | Contacts, competitors, action items |
| One of N choices | `aiSdk.Output.choice({ options })` | Lead classification, sentiment |

```typescript
// Classification → aiSdk.Output.choice
const { output: intent } = await generateText({
  prompt: 'classify_lead@v1',
  variables: { activity: leadActivity },
  output: aiSdk.Output.choice({ options: ['high', 'medium', 'low', 'unknown'] })
});

// Extraction → aiSdk.Output.object
const { output: company } = await generateText({
  prompt: 'extract_company@v1',
  variables: { content: websiteContent },
  output: aiSdk.Output.object({ schema: companySchema })
});

// List → aiSdk.Output.array
const { output: competitors } = await generateText({
  prompt: 'find_competitors@v1',
  variables: { companyName: 'Acme Corp', industry: 'SaaS' },
  output: aiSdk.Output.array({ element: z.object({ name: z.string(), reason: z.string() }) })
});
```
