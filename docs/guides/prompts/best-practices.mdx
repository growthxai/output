...
The examples below use [message mode](/prompts#prompt-body-modes): the rendered body starts with a role tag, and all top-level content stays inside supported role blocks. Use instruction mode instead when the prompt does not need conversation roles.
