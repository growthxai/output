...
- **`cost:llm:request`** — emitted when an LLM generation call finishes, with an LLM usage attribute.
...
A `cost:llm:request` event is emitted after text generation completes for `generateText()`, `generateTextWithStreaming()`, and Agent calls. For direct `streamText()` and `Agent.stream()` usage, the event fires when the stream finishes, not when it starts.
...
Cost is computed from the model's per-million-token pricing, fetched from a built-in pricing source and cached for 24 hours. For each priced dimension, the dollar amount is `(tokens / 1_000_000) * pricePerMillion`.

When the AI SDK input details reconcile with `inputTokens`, Output prices `noCacheTokens`, `cacheReadTokens`, and `cacheWriteTokens` separately. When output details reconcile with `outputTokens`, it prices `textTokens` and `reasoningTokens` separately. An incomplete breakdown falls back to aggregate input or output tokens. Missing cache or reasoning pricing falls back to the regular input or output rate.
...
| `input_cache_read` | Cached prompt read tokens (`inputTokenDetails.cacheReadTokens`). |
| `input_cache_write` | Prompt tokens written to cache (`inputTokenDetails.cacheWriteTokens`). |
| `output` | Text completion tokens. |
| `output_reasoning` | Reasoning tokens (`outputTokenDetails.reasoningTokens`). |

Only available, finite usage dimensions are included. When detailed counts are unavailable or inconsistent, `input` or `output` represents the aggregate token count instead.
