...
on<LLMUsageEvent>( 'cost:llm:request', async ( { eventId, workflowDetails, activityInfo, modelId, total, tokensUsed, usage } ) => {
...
on<HttpRequestCostEvent>( 'cost:http:request', async ( { eventId, workflowDetails, activityInfo, requestId, url, total } ) => {
  console.log( 'HTTP request', { eventId, workflowId: workflowDetails.workflowId, activityId: activityInfo.activityId, requestId, url, total } );
} );
```
