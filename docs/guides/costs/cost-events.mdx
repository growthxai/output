...
| `payload.cost` | `LLMGenerationCost \| null` | Cost calculated from that usage. A model or provider missing from the rate table gives an incomplete cost; `null` means no rate table could be built at all. |
