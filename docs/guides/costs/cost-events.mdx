...
`llm:generation:metering` is emitted after a generation ends for text, image, and Agent calls whenever usage is available. For direct `streamText()` and `Agent.stream()` usage, it fires when the stream finishes, not when it starts.

A failed generation also emits. Usage is collected as the call progresses, so a call that throws or is aborted after completing at least one step reports what it spent - an output that never validated, a tool loop that broke mid-run, an aborted stream, or a failure in your own code after the model answered. An event is therefore not proof of a successful call; a trace event carrying both usage and an error is a failure that still cost money. Failures before the first step completes report nothing, because no usage exists yet.

One event is emitted per call. Activity retries emit one per attempt, since each attempt calls the provider.

### Payload

The handler receives the standard event envelope. `payload.usage` is always present, and unpriced models produce an incomplete `payload.cost`:
...
| `payload.cost` | `LLMGenerationCost \| null` | Cost calculated from that usage. A model or provider missing from the rate table gives an incomplete cost. |
...
`LLMGenerationCost` contains one item for every normalized usage item. Cost is computed from per-million-token pricing fetched from the built-in pricing source and cached for 24 hours. Each item total is `(amount / 1_000_000) * ppm`. When that source is unreachable, rates come from the last cached copy or from a snapshot bundled with the package, and `pricingFreshness` records which.
