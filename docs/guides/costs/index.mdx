...
That's it. The command reads recorded trace cost attributes, applies any configured pricing overrides, and prints a breakdown:
...
1. **LLM calls** - Prefers the recorded `llm:generation:cost` attribute for the original cost. It can reprice `llm:generation:usage` with `costs.yml`, and still reads the retained legacy `llm:usage` attribute for compatibility with old and new traces.
