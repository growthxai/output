...
1. **LLM calls** - Prefers the recorded `llm:generation:cost` attribute for the original cost. It can reprice `llm:generation:usage` with `costs.yml`, and still reads legacy `llm:usage` attributes from older traces.
