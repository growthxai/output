...
Errors are emitted from three origins. Every payload includes `eventId`, `source`, and `error`. Activity- and workflow-scoped errors also include Temporal execution details.
...
- **Activity** — `eventId`, `source: 'activity'`, `activityInfo`, `workflowDetails`, `outputActivityKind`, `error`.
- **Workflow** — `eventId`, `source: 'workflow'`, `workflowDetails`, `error` (no activity fields).
