...
Errors thrown by steps and evaluators cross Temporal's Activity boundary before they reach workflow code. Temporal serializes them into a failure cause chain, so a direct `error instanceof CustomError` check is not reliable inside a workflow.

Use `hasErrorType(error, CustomError)` to inspect the complete cause chain. It matches native error instances as well as Temporal's serialized `type` and `name` fields.
