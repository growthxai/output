...
| Field | Description |
|-------|-------------|
| `id` | Unique identifier for this node. Steps, evaluators, and internal activities use `<activityId>:<attempt>`, so each retry of the same step is a distinct node |
...
When a failed step is retried, each attempt is traced separately: you get one node per attempt, sharing the same `name` but differing in the `:<attempt>` part of the `id`. This lets you compare what each attempt received, how it failed, and how long the backoff between attempts was.
