...
The evaluators on this page run _inside_ your workflows — they power generate-evaluate-retry loops in production. For testing workflow quality _across datasets_ without modifying your workflow code, see [Evaluation Workflow](/evaluators/workflow-evaluators). It covers `verify()` for creating typed evaluators, `Verdict` helpers for deterministic assertions, LLM judge functions, datasets, and running evals from the CLI.
...
- [Evaluation Workflow](/evaluators/workflow-evaluators) — Test workflow quality across datasets with `verify()`, `Verdict`, and the CLI
