...
When the worker starts, every env var set to `credential:<path>` is replaced with the decrypted value from `config/credentials.yml.enc`. By the time your first workflow runs, `ANTHROPIC_API_KEY` contains the real key — no code changes needed.
