...
You manage credentials through the [CLI](/packages/cli#credentials-commands) and read them in your steps with a simple dot-notation API.
...
When the worker starts, every env var set to `credential:<path>` is replaced with the value at that path in the global credentials for the current environment: `config/credentials/<NODE_ENV>.yml.enc` when it exists, otherwise `config/credentials.yml.enc`. Workflow-scoped credentials are not used for env vars. By the time your first workflow runs, `ANTHROPIC_API_KEY` contains the real key — no code changes needed.
...
import type { CredentialsProvider } from '@outputai/core/credentials';
...
Call `setProvider()` in a hook file listed under `outputai.hookFiles`. Hook files load before the worker resolves `credential:` env vars, so those resolve through your provider too.
...
Returning `null` from `loadForWorkflow` means the workflow has no overrides, so global credentials are used as-is.

## Security

Credentials are encrypted with **AES-256-GCM** using the `@noble/ciphers` library. Each encryption uses a fresh random nonce, so the same plaintext produces different ciphertext every time.

- Key files are created with file mode `0o600` (owner read/write only).
- During `output credentials edit`, the temporary plaintext file is overwritten with null bytes before it is deleted.
...
import {
  credentials,
  setProvider,
  MissingCredentialError, MissingKeyError, InvalidCredentialsKeyError, MalformedCredentialsKeyError
...
|--------|---------|--------|
| `credentials.get(path, defaultValue?)` | The value at `path`, or `defaultValue` (default `undefined`) when missing | Never for a missing path |
| `credentials.require(path)` | The value at `path` | `MissingCredentialError` when the value is missing or `null` |

`path` is a dot-notation path into the decrypted YAML, e.g. `'anthropic.api_key'`. Inside a step, the workflow's credentials are merged over the global ones. Credentials are decrypted once per scope and cached for the life of the process.

### Errors

All four errors extend `FatalError`, so a step that throws one fails without being retried.

| Error | Thrown when |
|-------|-------------|
| `MissingCredentialError` | `credentials.require()` finds no value at the path |
| `MissingKeyError` | No decryption key is found in the env var or key file |
| `InvalidCredentialsKeyError` | The key can't decrypt the file: it doesn't match, or the file is corrupted |
| `MalformedCredentialsKeyError` | The key isn't exactly 64 hex characters |

For complete TypeScript API documentation, see the [Credentials Module API Reference](https://output-ai-reference-code-docs.onrender.com/modules/core_src_credentials.html).
