...
2. Worker reads `output.hookFiles` from `package.json` and imports each listed file at startup. Projects scaffolded with `output init` include `node_modules/@outputai/credentials/dist/hooks.js`, which registers the credential resolution hook:
