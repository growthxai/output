...
This is the complement to [evaluators as steps](/evaluators) that run inside workflows. Those evaluators power generate-evaluate-retry loops in production. Workflow evaluators answer a different question: "across a set of known inputs, does my workflow still produce acceptable output?" For the full guide, see [Workflow Evaluators](/evaluators/workflow-evaluators).
