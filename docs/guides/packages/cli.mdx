...
If monitoring itself drops — the API restarts, the connection resets — the
workflow it started keeps running, and the command says so and exits `3` rather
than `1`. Retry logic keyed on a failed workflow therefore can't re-submit a
workflow that is already in flight:
