...
Calculate the estimated dollar cost of a workflow execution from its trace. Covers LLM token costs and API service costs. See [Cost Estimation](/operations/cost-estimation) for details on pricing configuration and the `config/costs.yml` override file.
...
<Tip>
If the report shows "Unknown models", add them to `config/costs.yml` in your project root. See [Cost Estimation — Pricing Configuration](/operations/cost-estimation#pricing-configuration) for the format.
</Tip>
