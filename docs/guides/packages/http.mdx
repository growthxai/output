...
Every HTTP call made through `@outputai/http`'s `fetch` (and the higher-level `httpClient`) emits an **`http:request`** hook event — independent of whether you attach a cost. Subscribe to it with [`on`](/packages/core#hooks) for logging, alerting, or per-vendor metrics:

```typescript
import { on } from '@outputai/core/hooks';

on( 'http:request', async ( payload ) => {
  // payload: { workflowId, runId, activityId?, requestId, method, url, status?, durationMs, outcome }
...
| Field | Description |
|-------|-------------|
| `workflowId` | Workflow id (stable across retries / continue-as-new). Auto-attached. |
| `runId` | Temporal run id for the current execution attempt. Auto-attached. |
| `activityId` | Activity / step id, when emitted from inside a step. Auto-attached. |
| `requestId` | UUID generated per request inside `@outputai/http`. |
