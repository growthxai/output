...
It also emits a **`cost:http:request`** hook event through the same hooks system as [LLM generation metering](/costs/cost-events#llm-generation-metering). For the payload and examples, see [Cost Events - HTTP](/costs/cost-events#http-request-cost).
