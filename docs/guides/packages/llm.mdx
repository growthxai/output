...
`@outputai/llm` ships built-in support for common AI SDK providers, but the provider packages themselves are optional peer dependencies. Install the package for each provider you use in prompt files:
...
For example, an OpenAI and Anthropic project should include:

```bash
npm install @outputai/llm @ai-sdk/openai @ai-sdk/anthropic
```

If a prompt uses a built-in provider whose package is not installed, `@outputai/llm` throws an error telling you which package to add.
