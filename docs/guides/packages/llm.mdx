...
| `cost` | Normalized `LLMCost` (`providerId`, `modelId`, aggregate totals, and priced items); `null` when pricing data is unavailable |
...
`streamText` / `Agent.stream()` `onEnd` receives the wrapped end payload: `result` (alias for `text`), `cost` (`null` when pricing is missing), and merged `sources` (always an array).
