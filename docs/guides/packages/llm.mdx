...
The prompt file is loaded and rendered at construction time. Variables and tools are fixed at construction. Skills come from the prompt file. The agent is ready to call `generate()` or `stream()` immediately.
...
| `onFinish` | `Function` | Finish event callback for `streamText` and `Agent.stream()` |
| `onError` | `Function` | Callback when a stream error occurs (`streamText` and `Agent.stream()`) |
