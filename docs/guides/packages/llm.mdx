...
Supported image inputs follow the AI SDK shape: `Buffer`, `Uint8Array`, `ArrayBuffer`, non-empty strings, or `{ data, mediaType }` objects. `mask` uses the same input shape and requires `images`.
