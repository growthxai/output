...
| `cost` | Normalized `LLMGenerationCost` (`providerId`, `modelId`, aggregate totals, and priced items). A model or provider missing from the rate table yields an incomplete cost; `null` when usage cannot be normalized |

The `cost` property is an `LLMGenerationCost`:
...
Detailed cost items are used when the provider breakdown reconciles with the aggregate token count. Missing cache or reasoning pricing falls back to the regular input or output rate and marks the item as `fallback`. Missing prices produce `null` item costs and an `incomplete` aggregate status. `pricingFreshness` reports how current the rate table was (`live`, `cached`, `stale`, or `snapshot`).
...
`streamText` / `Agent.stream()` `onEnd` receives the wrapped end payload: `result` (alias for `text`), `cost` (an incomplete `LLMGenerationCost` when model or dimension pricing is missing, `null` when usage cannot be normalized), and merged `sources` (always an array).
...
LLM cost estimation looks up rates by `provider` + `model` in the [models.dev](https://models.dev) catalog. Built-in provider names match that catalog. A custom `registerProvider` name (such as `vertex-anthropic` above) will not match, so `response.cost` is an incomplete `LLMGenerationCost` with unavailable totals and a missing-pricing warning is logged - register under a models.dev provider id if you need automatic pricing.

The catalog is fetched once and cached for a day. If a refresh fails, the previously cached rates are reused; if the catalog is unreachable before anything has been cached, the built-in providers fall back to a snapshot of the catalog bundled with `@outputai/llm`. Every cost records which of these it used in `pricingFreshness`. Because the snapshot is point-in-time and can lag the live catalog by an unbounded amount, a cost priced from it is reported as `imprecise`; a merely stale cache is not, since those rates were accurate when they were fetched. Custom registered providers are not in the snapshot and get unavailable totals as described above. The snapshot is refreshed periodically, so upgrading to a newer `@outputai/llm` picks up a more recent table.
...
## AI SDK telemetry integrations

Output passes its own per-call `telemetry.integrations` to every text and stream call, which is how usage is metered. The AI SDK gives per-call integrations precedence over globally registered ones, so integrations registered with `aiSdk.registerTelemetry()` - OpenTelemetry, Langfuse, AI DevTools - do not receive lifecycle events from `@outputai/llm` calls. They still work for AI SDK calls you make directly.

Provider-level instrumentation is unaffected, and Output's own tracing covers every LLM call.
...
`llm:generation:metering` is emitted whenever raw AI SDK usage can be normalized. Its payload contains `usage: LLMGenerationUsage` and `cost: LLMGenerationCost | null`. Missing model or dimension prices produce an incomplete `LLMGenerationCost`.
...
`cost:llm:request` remains available with its legacy `LLMUsageEvent` payload, so existing hooks do not need a structural migration. The same payload remains attached to priced LLM traces as the deprecated `attributes["llm:usage"]` attribute, preserving existing trace readers.
...
Legacy usage line types are frozen as `input`, `input_cached`, `output`, and `reasoning`. The event and trace attribute keep this payload shape for compatibility, while corrected reasoning accounting or AI SDK 7 usage details can change individual values. `LLMUsageEvent` is deprecated only to guide new integrations toward the more complete event and normalized trace attributes.
