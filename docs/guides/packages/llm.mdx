...
The catalog is fetched once and cached for a day. If a refresh fails, the previously cached rates are reused; if the catalog is unreachable before anything has been cached, the built-in providers fall back to a snapshot of the catalog bundled with `@outputai/llm`. Every cost records which of these it used in `pricingFreshness`. Because the snapshot is point-in-time and can lag the live catalog by an unbounded amount, a cost priced from it is reported as `imprecise`; a merely stale cache is not, since those rates were accurate when they were fetched. Custom registered providers are not in the snapshot and get unavailable totals as described above.
