...
Detailed cost dimensions are used when the provider breakdown reconciles with the aggregate token count. Missing cache or reasoning pricing falls back to the regular input or output rate; incomplete breakdowns fall back to aggregate `input` or `output` usage.
...
Cache activity appears in the response usage and the [cost event](/costs/cost-events): the first call reports cache creation in `usage.inputTokenDetails.cacheWriteTokens`, and later calls within the TTL report cache reads in `usage.inputTokenDetails.cacheReadTokens`. The corresponding `response.cost.usage` entries use `input_cache_write` and `input_cache_read`.
