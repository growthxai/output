...
A failure in the SDK's own response handling - reading the model response, merging sources, building the returned response - is not retried: it throws a non-retryable `FatalError` wrapping the original error, so Temporal fails the activity on the first attempt. Such a failure is deterministic, and a retry would pay for the model call again and fail the same way. The usage already spent is still billed and recorded on the trace.
