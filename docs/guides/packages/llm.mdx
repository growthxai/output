...
```typescript
import { generateTextWithStreaming, aiSdk } from '@outputai/llm';
...
The prompt file is loaded and rendered at construction time. Variables and tools are fixed at construction. Skills and `maxSteps` come from the prompt file. The agent is ready to call `generate()` or `stream()` immediately.
...
| `experimental_transform` | `Function` | Stream transform, e.g. `aiSdk.smoothStream()` (streaming APIs) |

Options set in the prompt file (`temperature`, `maxTokens`) can be overridden at call time. `maxSteps` is prompt-owned; pass `stopWhen` to override the loop ceiling.
