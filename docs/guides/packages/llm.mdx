...
Each call seeds authored `<user>` blocks from the prompt. `<system>` blocks become `instructions`. Authored `<assistant>` and `<tool>` blocks are dropped; use `generateText` when the prompt is a few-shot or prefilled thread.
