...
The `@outputai/llm` package is how you call LLMs from your steps and evaluators. It wraps the [AI SDK](https://sdk.vercel.ai/docs) and adds [prompt files](/prompts) — version-controlled `.prompt` files that live alongside your code and define the provider, model, temperature, and prompt template in one place.

## Generate Functions

`generateText` is the primary function for LLM calls. Use the `output` parameter with `Output.*` helpers to control the response shape. When you need progress as text arrives, prefer `generateTextWithStreaming` in workflow steps: it uses streaming internally but returns a complete result and rejects on provider or transport errors. Use `streamText` when you need direct control over the stream:
...
Use `onFinish` and `onError` for observation or side effects. Errors are still thrown after `onError` runs:
...
`generateTextWithStreaming` also supports structured output. Pass an `Output.*` specification and read the parsed value from `result.output`, just as with `generateText`.
...
Both streaming APIs accept stream transforms such as `smoothStream` for more natural output pacing:

```typescript
import { generateTextWithStreaming, smoothStream } from '@outputai/llm';
...
Use `generateWithStreaming()` when you want streaming progress and a complete result:
...
const result = await agent.generateWithStreaming({
  ...
  onFinish(response) {
    // Receives the same complete result shape returned by generateWithStreaming()
    console.log('Generation complete', response.result, response.usage);
  },
  onError({ error }) {
    console.error('Generation failed', error);
...
Like `generate()`, the method returns the complete response, rejects on stream errors, and automatically appends messages to the configured conversation store. Prefer it over `stream()` in workflow steps unless you need direct access to the stream result.

### stream()

Use `stream()` when you need direct access to the agent's stream result:
...
<Note>
`generate()` and `generateWithStreaming()` automatically append messages to the conversation store. `stream()` does not; if you use direct streaming with a conversation store, persist messages manually in the `onFinish` callback.
</Note>
...
Generation APIs accept additional [AI SDK options](https://sdk.vercel.ai/docs) passed through to the provider. Availability depends on whether the API uses streaming:

| Option | Type | Description |
|--------|------|-------------|
| `tools` | `ToolSet` | Tools the model can call |
...
| `onChunk` | `Function` | Callback for each chunk (streaming APIs) |
| `onFinish` | `Function` | Complete response callback for `*WithStreaming`; finish event callback for direct streams |
| `onError` | `Function` | Callback when a stream error occurs (streaming APIs) |
| `experimental_transform` | `Function` | Stream transform, e.g. `smoothStream()` (streaming APIs) |

Options set in the prompt file (temperature, maxTokens) can be overridden at call time.

### Retries and Network Timeouts

`generateText`, `generateTextWithStreaming`, and `streamText` set `maxRetries: 0` by default. In Output workflows, LLM calls usually run inside steps, and steps are Temporal activities. When a provider error is allowed to fail the step, Temporal records the failed activity attempt and retries it according to the workflow's retry policy.
