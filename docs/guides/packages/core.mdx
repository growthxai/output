...
## Temporal Access from Steps

Steps and evaluators run as Temporal Activities inside the worker's Node process. The `@outputai/core/temporal` subpath gives them direct access to the Temporal layer, sharing the worker's live connection — no extra configuration, TLS, or credentials handling.

### signalInvokingWorkflow

Send a signal to the workflow run that invoked the current step. The primary use case is streaming incremental results — e.g. LLM token batches — back to a workflow that buffers them for a client-facing update handler:

```typescript steps.ts
import { step } from '@outputai/core';
import { signalInvokingWorkflow } from '@outputai/core/temporal';
...
export const draftWithProgress = step({
  ...
  fn: async (input) => {
    const stream = streamText({ prompt: 'draft@v1', variables: input });
    for await (const chunk of stream.textStream) {
      await signalInvokingWorkflow('progressToken', chunk);
    }
...
The target is resolved from the activity context and pinned to the exact `workflowId` + `runId`, so a stale activity from a retried or superseded run can never signal a newer run reusing the same workflow id. If the invoking run has already completed, the signal rejects and the activity fails — surfacing the problem instead of signaling into the void.

### getWorkflowClient

For anything beyond signaling the invoking workflow, `getWorkflowClient()` returns a full Temporal [`Client`](https://typescript.temporal.io/api/classes/client.Client) that shares the running worker's connection and namespace:

```typescript
import { getWorkflowClient } from '@outputai/core/temporal';
...
The client is memoized and reuses the worker's live connection — do not close it.

<Warning>
Both helpers are only available inside steps/evaluators executed by a running Output worker; elsewhere (e.g. unit tests) they throw a non-retryable `FatalError`. Never import `@outputai/core/temporal` from `workflow.ts` — workflows run in Temporal's isolated sandbox and cannot hold connections, so the import would fail the workflow bundle.
</Warning>
