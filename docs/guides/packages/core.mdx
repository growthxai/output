...
Every hook payload includes an **`eventId`** — a UUID v4 stamped per emit. Use it as a stable per-emit idempotency key for downstream dedup (webhook retry handling, ClickHouse `ReplacingMergeTree`, audit logs, etc.). Distinct emits — including `http:request` and `cost:http:request` for the same fetch — receive distinct `eventId`s.

| Function | When it runs | Payload |
|----------|--------------|---------|
| `onError` | Activity, workflow, or runtime error | Object always includes `eventId`, `source`, and `error`. Other fields depend on `source` — see below and [Error Hooks](/operations/error-hooks) |
| `onBeforeWorkerStart` | Once after hooks, workflows, and activities are loaded; before the Temporal worker is created | None |
| `onWorkflowStart` | A user workflow execution starts | `eventId`, `id` (workflow id), `runId` (Temporal run id), `name` (workflow type) |
| `onWorkflowEnd` | A user workflow execution completes successfully | `eventId`, `id`, `runId`, `name`, `duration` (ms) |
| `onWorkflowError` | A user workflow execution fails | `eventId`, `id`, `runId`, `name`, `duration` (ms), `error` |
| `on(eventName, handler)` | Custom events emitted elsewhere in the stack | Optional object. For built-in events emitted via `emitEvent` (e.g. `http:request`, `cost:llm:request`), the framework auto-attaches `eventId`, `workflowId`, `runId`, and `activityId` to the payload. |

**`onError` payload by `source`**

- **`activity`** — `eventId`, `source`, `activityId`, `activityName`, `workflowId`, `workflowName`, `error`
- **`workflow`** — `eventId`, `source`, `workflowId`, `workflowName`, `error`
- **`runtime`** — `eventId`, `source`, `error`
...
| `TEMPORAL_GRPC_MAX_MESSAGE_SIZE_BYTES` | `33554432` (32 MiB) | Max gRPC message size (send + receive) for the worker's Temporal connection. Raise if large payloads hit `RESOURCE_EXHAUSTED`. |
