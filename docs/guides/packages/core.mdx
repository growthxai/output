...
| `onWorkflowStart` | A user workflow execution starts | `eventId`, `workflowDetails` |
| `onWorkflowEnd` | A user workflow execution completes successfully | `eventId`, `workflowDetails` |
| `onWorkflowError` | A user workflow execution fails | `eventId`, `workflowDetails`, `error` |
| `on(eventName, handler)` | Custom events emitted elsewhere in the stack | Your event payload plus `eventId`. When emitted from an Output activity, the framework also attaches `activityInfo`, `workflowDetails`, and `outputActivityKind`. |
...
- **`activity`** — `eventId`, `source`, `activityInfo`, `workflowDetails`, `outputActivityKind`, `error`
- **`workflow`** — `eventId`, `source`, `workflowDetails`, `error`
