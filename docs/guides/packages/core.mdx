...
| `onWorkflowStart` | A user workflow execution starts | `id` (run id), `name` (workflow type) |
| `onWorkflowEnd` | A user workflow execution completes successfully | `id`, `name`, `duration` (ms) |
| `onWorkflowError` | A user workflow execution fails | `id`, `name`, `duration` (ms), `error` |
| `on(eventName, handler)` | Custom events emitted elsewhere in the stack | Optional object |
