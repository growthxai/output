...
`SIGTERM`, `SIGINT`, and `SIGUSR2` start a graceful shutdown: the worker stops polling, lets in-flight activities run for `TEMPORAL_SHUTDOWN_GRACE_TIME` before asking them to cancel, gives up draining after `TEMPORAL_SHUTDOWN_FORCE_TIME`, then closes its Temporal connection. A second signal forces an immediate exit.

Uncaught exceptions and unhandled rejections take the same path, so in-flight work still drains, with a forced exit if the drain stalls.

| Exit code | Meaning |
|-----------|---------|
| `0` | A signal requested the shutdown. Failures while draining are logged as warnings and do not change the code |
| `1` | Startup failed, the Temporal connection was lost, an uncaught error ended the process, or a second signal forced an immediate exit |
...
| `TEMPORAL_SHUTDOWN_GRACE_TIME` | `20s` | Time to let in-flight work drain before activities are asked to cancel. Accepts milliseconds or duration strings like `15s`. |
| `TEMPORAL_SHUTDOWN_FORCE_TIME` | `25s` | Time before the worker gives up draining and terminates. Accepts milliseconds or duration strings like `15s`. |
