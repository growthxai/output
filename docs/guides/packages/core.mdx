...
|----------|--------------|---------|
| `onError` | Activity, workflow, or runtime error | Object always includes `source` and `error`. Shape depends on `source` — see below and [Error Hooks](/operations/error-hooks) |
| `onBeforeWorkerStart` | Once after hooks, workflows, and activities are loaded; before the Temporal worker is created | None |
| `onWorkflowStart` | A user workflow execution starts | `id` (workflow id), `runId` (Temporal run id), `name` (workflow type) |
| `onWorkflowEnd` | A user workflow execution completes successfully | `id`, `runId`, `name`, `duration` (ms) |
| `onWorkflowError` | A user workflow execution fails | `id`, `runId`, `name`, `duration` (ms), `error` |
| `on(eventName, handler)` | Custom events emitted elsewhere in the stack | Optional object. For built-in events emitted via `emitEvent` (e.g. `http:request`, `cost:llm:request`), the framework auto-attaches `workflowId`, `runId`, and `activityId` to the payload. |

**`onError` payload by `source`**

- **`activity`** — `source`, `activityId`, `activityName`, `workflowId`, `workflowName`, `error`
- **`workflow`** — `source`, `workflowId`, `workflowName`, `error`
