...
`SIGTERM`, `SIGINT`, and `SIGUSR2` start a graceful shutdown: the worker stops polling, lets in-flight activities finish within `TEMPORAL_SHUTDOWN_GRACE_TIME`, then closes its Temporal connection. A second signal forces an immediate exit.
