...
| `TransparentFatalError` | Non-retryable wrapper that surfaces its original error |
| `ValidationError` | Schema validation failure — stops immediately |
| `hasErrorType` | Match an error class across a Temporal failure cause chain |
...
4. **Publishes the catalog** so the API can resolve workflow names for this version of your code
5. **Starts processing** workflow executions

Publishing the catalog gates step 5. If the worker cannot publish a catalog matching its own source code, it retries and then exits rather than serving requests against an outdated catalog.
...
| Exit code | Meaning |
|-----------|---------|
| `0` | A signal requested the shutdown and the worker drained inside `TEMPORAL_SHUTDOWN_FORCE_TIME`. A service other than the worker failing to stop is logged as a warning and does not change the code |
| `1` | Startup failed, the worker failed to stop for any reason - including in-flight work that did not finish before `TEMPORAL_SHUTDOWN_FORCE_TIME` - the Temporal connection was lost, an uncaught error ended the process, or a second signal forced an immediate exit |
...
**Keep handlers fast.** Every handler is tracked while it runs, and a [shutdown](#shutdown) awaits all the pending ones together under a single `OUTPUT_HOOK_FLUSH_TIMEOUT_MS` budget. There is no opt-out, so one handler awaiting a slow network call spends the allowance every other pending handler needs, and they can all be dropped together. Write to stdout or an in-process buffer and let your log pipeline or a sidecar ship the data. If a handler genuinely has to await remote I/O, batch it and raise `OUTPUT_HOOK_FLUSH_TIMEOUT_MS` along with your platform's shutdown window.
...
| `onActivityEnd` | An Output activity completes successfully, including internal activities | `eventId`, `eventDate`, `activityInfo`, `workflowDetails`, `outputActivityKind` |
| `onActivityError` | An Output activity fails, including internal activities | `eventId`, `eventDate`, `activityInfo`, `workflowDetails`, `outputActivityKind`, `error` |
...
The `@outputai/core/credentials` subpath reads encrypted secrets from `credentials.yml.enc` files, scoped globally, per environment, or per workflow. Use it in steps and other worker-side code:
...
The worker also resolves `credential:<path>` values in environment variables at startup. See [Credentials](/packages/core/credentials) for setup, key management, and deployment.
...
Get a handle pinned to the exact workflow run that invoked the current step. The primary use case is signaling incremental results — e.g. LLM token batches — back to a workflow that buffers them for a client-facing update handler:

```typescript steps.ts
import { step } from '@outputai/core';
import { getCurrentWorkflowHandle } from '@outputai/core/temporal';
import { generateTextWithStreaming } from '@outputai/llm';

export const draftWithProgress = step({
  name: 'draftWithProgress',
  inputSchema: DraftInput,
  outputSchema: DraftOutput,
  fn: async (input) => {
    const handle = getCurrentWorkflowHandle();
    const result = await generateTextWithStreaming({
      prompt: 'draft@v1',
      variables: input,
      async onChunk({ chunk }) {
        if (chunk.type === 'text-delta') {
          await handle.signal('progressToken', chunk.text);
        }
      }
    });
...
For operations on other workflows, `createTemporalClient()` creates a full Temporal [`Client`](https://typescript.temporal.io/api/classes/client.Client) that shares the running worker's connection and namespace:

```typescript
import { createTemporalClient } from '@outputai/core/temporal';

const client = createTemporalClient();
const handle = client.workflow.getHandle(someWorkflowId);
await handle.signal('resume', payload);
```

Each call creates a lightweight client while reusing the worker's live connection. Do not close `client.connection`; the worker owns its lifecycle.

<Warning>
`createTemporalClient()` is available after the worker establishes its Temporal connection; code that runs while hook files load runs before that point. `getCurrentWorkflowHandle()` additionally requires a Temporal Activity context. Elsewhere (e.g. unit tests), they throw a non-retryable `FatalError`; mock the helpers in those tests. Never import `@outputai/core/temporal` from `workflow.ts` — workflows run in Temporal's isolated sandbox and cannot hold connections, so the import would fail the workflow bundle.
</Warning>

<Warning>
Prefer normal dataflow (step return values) whenever the workflow only needs the final result. Signal-per-chunk streaming adds one history event per signal; batch chunks aggressively, and for high-frequency token streams consider an out-of-band channel (e.g. Redis pub/sub) with only the final result returned through Temporal.
</Warning>
...
| `TEMPORAL_SHUTDOWN_GRACE_TIME` | `15s` | Time to let in-flight work drain before activities are asked to cancel. Accepts milliseconds or duration strings like `15s`. |
| `TEMPORAL_SHUTDOWN_FORCE_TIME` | `20s` | Time before the worker gives up draining and terminates. Accepts milliseconds or duration strings like `15s`. |
| `OUTPUT_HOOK_FLUSH_TIMEOUT_MS` | `5000` (5s) | Time to await pending hook callbacks after the drain, before exiting without them. Spent after `TEMPORAL_SHUTDOWN_FORCE_TIME`, so both count against your platform's shutdown window. |
