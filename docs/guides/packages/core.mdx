...
Some metadata field names are reserved because Winston, Output log hooks, or log formatting use them internally. Do not use these keys in logger metadata:

`label`, `level`, `message`, `metadata`, `namespace`, `splat`, `stack`, and `timestamp`.
