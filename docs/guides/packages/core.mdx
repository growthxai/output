...
<Warning>
Avoid putting secrets in workflow HTTP requests. Values in `url`, `payload`, literal `headers`, and activity responses can be stored in Temporal history and trace files. Use `$ENV_VAR_NAME` placeholders for request header secrets, avoid secrets in URLs and payloads, and only set `responseOptions.includeBody: true` when the response body is safe to persist.
</Warning>
