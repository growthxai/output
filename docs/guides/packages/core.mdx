...
|----------|--------------|---------|
| `onError` | Activity, workflow, or runtime error | `source` (`activity`, `workflow`, or `runtime`), `error`, and when applicable `workflowName` / `activityName` — see [Error Hooks](/operations/error-hooks) |
