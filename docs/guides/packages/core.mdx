...
| Exit code | Meaning |
|-----------|---------|
| `0` | A signal requested the shutdown and the worker drained. Another service failing to stop is logged as a warning and does not change the code |
| `1` | Startup failed, the worker failed to drain, the Temporal connection was lost, an uncaught error ended the process, or a second signal forced an immediate exit |
