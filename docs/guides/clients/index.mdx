...
This captures everything flowing through `outputFetch` and `createKyClient` — request bodies, response bodies, headers, and timing. Sensitive headers like `authorization` and `api-key` are automatically redacted so you can safely share traces.
...
- [@outputai/http API reference](/packages/http) — `outputFetch`, `createKyClient`, and configuration options
