...
**`createClient()`** builds an HTTP client with a base URL and timeout. Every request through this client is automatically traced — you'll see the URL, status code, and timing in your workflow trace. The `prefix` means you only write relative paths like `'search'` instead of the full URL.
...
const createClient = () =>
  createKyClient({
    prefix: 'https://company.clearbit.com/v2',
    timeout: 30000,
    headers: {
...
Ky clients throw two common error types:
- **`ky.HTTPError`** — The server responded with a non-2xx status code (like 404, 429, or 500)
- **`ky.TimeoutError`** — The request took longer than the timeout you set
...
export const searchClient = {
  query: async (q: string): Promise<SearchResult[]> => {
    ...
    try {
      ...
      // Everything else (5xx, ky.TimeoutError): let it propagate.
...
Steps import from client files — they never create `createKyClient` instances directly. When an API changes its auth or base URL, you update one file and every workflow that uses it picks up the change.
...
This captures everything flowing through `instrumentedFetch` and `createKyClient` — request bodies, response bodies, headers, and timing. Sensitive headers like `authorization` and `api-key` are automatically redacted so you can safely share traces.
...
- [@outputai/http API reference](/packages/http) — `instrumentedFetch`, `createKyClient`, and configuration options
