...
Individual steps can override these with their own `options.activityOptions` — see [step options](/steps#options). When this workflow is called as a child workflow, the caller can also pass invocation-level `activityOptions` for that child branch; step options still take final precedence. See [Child Workflows](/workflows/child-workflows#activity-options-override).

The fields under `activityOptions` are supported by Output and relayed directly to [Temporal](https://temporal.io/), our runtime. Output accepts Temporal [Activity options](https://typescript.temporal.io/api/interfaces/common.ActivityOptions), except `versioningIntent`, `taskQueue`, and `allowEagerDispatch`:
...
Custom `nonRetryableErrorTypes` are added alongside `FatalError`; activity options cannot make `FatalError`, `ValidationError`, or `TransparentFatalError` retryable.
