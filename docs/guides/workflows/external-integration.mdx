...
<Warning>
Do not put keys, tokens, passwords, cookies, signed URLs, or other secrets directly in `url`, `headers`, `payload`, or `responseOptions.includeBody` responses. Workflow activity inputs and outputs are durable history and may be visible in Temporal tooling and trace files. Response headers are redacted when included, but request headers are only protected if you use `$ENV_VAR_NAME` placeholders.
</Warning>
...
Header values can reference worker environment variables with `$ENV_VAR_NAME`, for example `Authorization: 'Bearer $API_TOKEN'`. Response headers are redacted when included. Response bodies are not redacted, so only set `includeBody: true` when the body is safe to store in workflow history and trace files.
