  ...
  describe( 'wrapStream', () => {
    ...
    it( 'starts an llm trace, returns fn result, and passes hooks', () => {
      ...
      expect( fn ).toHaveBeenCalledWith( {
        ...
        onStepEndHook: expect.any( Function )
      } );
    ...
    it( 'records the steps it is given', async () => {
      const step = { usage: { inputTokens: 2, outputTokens: 1 } };
      const { onStepEndHook } = hooksFrom( 'streamText' );

      await onStepEndHook( step );
