...
- a4f6bd4: Fixed two defects that made trace trees merge or drop nodes.
...
- 38c895c: Updated error serialization to keep only `name`, `message`, `code` and `stack` for `DOMException` values, instead of also projecting the 25 legacy numeric constants their prototype carries. Aborted operations, whose reason is usually a `DOMException`, now read like any other error in traces and logs.

## 0.13.0

### Minor Changes

- 4f4bf8f: Hardened worker startup and shutdown around the catalog workflow.
...
- a5da6b5: - Workflow errors passed to `onWorkflowError` and workflow-sourced `onError` are rehydrated to `Error` instances after crossing the Temporal sandbox (preserving `name`, `message`, `cause`, and other enumerable properties).
...
- 47f491f: - Added `@outputai/core/temporal` for worker-side Temporal access that shares the running worker's connection and namespace;
