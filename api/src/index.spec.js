...
describe( 'API endpoints', () => {
  ...
  describe( 'GET /workflow/:id/history/stream', () => {
    const makeStreamGen = workflow => ( async function *() {
      yield { type: 'workflow', workflow };
      yield { type: 'done', reason: 'WORKFLOW_EXECUTION_COMPLETED' };
    } )();

    it( 'returns 200 with text/event-stream content type', async () => {
      mockClient.streamWorkflowHistory.mockReturnValue( makeStreamGen( {
    ...
    it( 'calls streamWorkflowHistory with correct workflowId', async () => {
      ...
      await request( `http://localhost:${PORT}` )
        .get( '/workflow/w1/history/stream' )
        .expect( 200 );
      expect( mockClient.streamWorkflowHistory ).toHaveBeenCalledWith( 'w1', expect.any( Object ) );
    } );
    ...
    it( 'pinned run variant calls streamWorkflowHistory with path runId', async () => {
      ...
      await request( `http://localhost:${PORT}` )
        ...
        .expect( 200 );
      expect( mockClient.streamWorkflowHistory ).toHaveBeenCalledWith( 'w1', expect.objectContaining( { runId: RID } ) );
    } );
  } );
